...
class Con_ResBlock(TimeConstepBlock):
    ...
    def forward(self, x, emb, z_sem):
        ...
        """
        return checkpoint(
            self._forward, (x, emb, z_sem), self.parameters(), self.use_checkpoint
        )
...
class Con_UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None, low_cond = None, z_sem = None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        for module in self.input_blocks:
            if isinstance(z_sem, dict):
                if 'my_opt' in z_sem and isinstance(module[0], Con_ResBlock):
                    h = module(h, emb, z_sem['my_opt']['input_block'][i])
                    i = i + 1
                else:
                    h = module(h, emb, z_sem)
            else:
                h = module(h, emb, z_sem)
            hs.append(h)
        if isinstance(z_sem, dict):
            if 'my_opt' in z_sem:
                h = self.middle_block[0](h, emb, z_sem['my_opt']['middle_block'][0])
                h = self.middle_block[1](h)
                h = self.middle_block[2](h, emb, z_sem['my_opt']['middle_block'][1])

        else:
            h = self.middle_block(h, emb, z_sem)
        j = 0
        tmp_output_feature = None
        for module in self.output_blocks:
            if hs[-1].size(-1) < h.size(-1):
                h = h[..., :-1]
            if hs[-1].size(-2) < h.size(-2):
            ...
            if isinstance(z_sem, dict):
                if 'my_opt' in z_sem and isinstance(module[0], Con_ResBlock):
                    h = module(cat_in, emb, z_sem['my_opt']['output_block'][j])
                    j = j + 1
                else:
                    h = module(h, emb, z_sem)

            else:
                h = module(cat_in, emb, z_sem)
        h = h.type(x.dtype)
        return self.out(h)
